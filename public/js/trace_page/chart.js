...
$("#add-file").click(function(){
	...
	if (select_element.value != "none" && selected_files.indexOf(select_element.value) < 0 && selected_files.length < 1) {
		const file_selected_div = document.getElementById("files-selected-list");
		selected_files.push(select_element.value);
		let file_select_content = "<div class='row' id='" + select_element.value + "'><div class='col-xs-10'><p class='content-test'>";
		file_select_content += select_element.value + "</p></div><div class='col-xs-2'><button class='btn btn-danger' id='remove-selected-file'>Remove</button></div></div>";

		// creating an HTML element and setting the HTML string as innerHTML then appending it to the list DOM
		let template = document.createElement('div');
		template.id = "selected-" + select_element.value;
		template.innerHTML = file_select_content;

		file_selected_div.append(template);
		loadData(select_element.value);
	}
...
function loadData(file_name) {
	//resetGraph();
	d3.json(`https://s3.us-east-2.amazonaws.com/fstraces/${id}/${file_name}/final.json`, function (error, d) {
